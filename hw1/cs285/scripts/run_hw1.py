    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                expert_actions = expert_policy.get_action(paths[itr]["observation"])
                paths[itr]["action"] = expert_actions
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          permutation = np.random.permutation(len(replay_buffer.obs))
          ob_batch, ac_batch = (replay_buffer.obs[permutation][:params["train_batch_size"]],
                                replay_buffer.acs[permutation][:params["train_batch_size"]])
        ...
        if log_metrics:
            ...
            logger._summ_writer.add_text("params", str(params), global_step=0)
            print('Done logging...\n\n')
